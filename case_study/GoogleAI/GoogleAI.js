...
//            console.log("[*] onLeave: g_BpHwBinder_transact_vendor_ptr");
    ...
    const g_BpHwBinder_transact_ptr = Module.getExportByName(
        "/system/lib64/libhidlbase.so",
        '_ZN7android8hardware10BpHwBinder8transactEjRKNS0_6ParcelEPS2_jNSt3__18functionIFvRS2_EEE');
    console.log("[i] BpHwBinder::transact ptr addr: " + g_BpHwBinder_transact_ptr)

    Interceptor.attach(g_BpHwBinder_transact_ptr, {
        onEnter: function(args) {
            console.log("[*] onEnter: g_BpHwBinder_transact_ptr");
            parseParcel(args);
        },
...
//            console.log("[*] onLeave: g_BpHwBinder_transact_vendor_ptr");
    ...
    var g_BpBinder_transact_ptr = Module.getExportByName("libbinder.so", '_ZN7android8BpBinder8transactEjRKNS_6ParcelEPS1_j');
    console.log("[i] BpBinder::transact addr: " + g_BpBinder_transact_ptr)
    Interceptor.attach(g_BpBinder_transact_ptr, {
        onEnter: function(args) {
            // console.log("[*] onEnter: g_BpBinder_transact_ptr")
            // mData
            var mData_offset = args[2].add(0x8);
            var mData_pos = mData_offset.readPointer();

            // for collecting the black list
            var interface_token = mData_pos.add(0xc).readUtf16String();
            if (!black_interface_list.includes(interface_token))
                console.log(interface_token);
        },
